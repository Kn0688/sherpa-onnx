...
- `docs/fireredasr-cuda-optimization.md` — **CUDA platform companion** (GTX 1660 SUPER server, synced from `fireredasr-batch-decoding`): why split-format int8 fails on CUDA EP (535 host↔device memcpys), fp16 conversion via `scripts/fire-red-asr/convert-fp16.py` (encoder 2.67x vs int8, text-identical), MAX_BATCH=0 flip, fp32 encoder swap (2026-09-17: fp16 GEMM ~6x slower than fp32 on TU116's non-tensor-core cublas; earlier "47% wall-clock gap" was an ORT profiler misreading — nsys shows GPU 97.8% busy; RTF 0.098→0.036, md5 byte-identical; decoder stays fp16, N=1 decode is bandwidth-bound), BFC arena opt-out `SHERPA_ONNX_CUDA_USE_ARENA=0` (raw cudaMalloc env allocator; fixes fp32 3h48 OOM at 56%: 797s vs fp16 1720s, VRAM sawtooth 4337-4797 MiB, e2e +2.8%), dead ends, remaining CUDA roadmap (batch direction closed 2026-09-18: batch path was already GPU-resident — "per-step PCIe tax" premise wrong; fp32-decoder batching works (160s RTF 0.026, 1.42x, md5-identical) but needs VAD cap 10s on 6GB — rejected per no-truncation constraint, reverted). zh chain cumulative: RTF 0.318 → 0.037 (8.6x), 3h48 RTF 0.057
...
- Branch `fireredasr-mobile` (fork, `d00c7927`, based on `ios-fireredasr-cache-opt`) — **current on-device working branch, and the release branch collecting ALL optimizations**: adaptive cache + enc_mask/cross_mask passthrough (works with released AND batch-capable models) + 8 tokens/s cap, NO batch code. Batch is parked on `fireredasr-batch-decoding` after iOS jetsam kills (bug_type 298) at 3.54GB RSS on long transcriptions — desktop batch peak is only ~1.9GB, so device growth is likely ORT arena retention across varied batch shapes; planned fix if revived: periodic recognizer recreation to flush arena. CPU-watchdog theory (bug_type 202 cpu_resource report, 66%/50%) was ruled out — 1-thread batch also died. PR to upstream: branch `fireredasr-adaptive-kv-cache` (cache-only, minimal)
  - **2026-08-13 sync: now also carries the full Whisper optimization set** (commits `92258ec5` adaptive self-KV cache, `448acc6e` 6→8 tokens/s fast-speech truncation fix, `e7457940` export optimizations — TextDecoderSlim slim decoder graph 13825→3261 nodes, logits-projection constant folding so quantize_dynamic covers it, decoder int4 MatMulNBits with `--no-int4` opt-out; measured on medium: dense-English e2e decode -28%, decoder file -17.7%, text token-identical; recommended deployment `medium-encoder.int8.onnx` + `medium-decoder-slim.int4.onnx`). iOS/macOS onnxruntime pinned 1.27.0 supports MatMulNBits/KleidiAI; Android AAR needs ORT >= 1.17 for int4, else fall back to the slim int8 decoder
  - **2026-08-13 sync: `scripts/fire-red-asr/` copied byte-identical from `fireredasr-batch-decoding`** (`357696be`) — full model-production tooling including the conv→MatMul pointwise rewrite (int8 encoder -30%, weights -37%) and int4/int8/QDQ quantizers. Runtime int4 models require ORT >= 1.17 (iOS 1.27.0 OK)
  - **2026-08-13: parakeet-tdt-0.6b-v3 int8 encoder optimization** — `scripts/nemo/parakeet-tdt-0.6b-v3/optimize_encoder_int8.py` (+ README there). Graph surgery on the released int8 encoder (no re-export): 77 ConvInteger (no optimized arm64 kernel, 28% of encoder time) rewritten by type — 24 depthwise(k=9)+5 subsampling → fp32 Conv with offline-dequantized weights, 48 pointwise 1x1 → Transpose+MatMulInteger+Transpose keeping ORIGINAL u8 weights (bit-exact, fast MLAS u8 GEMM). Output same size, decoder/joiner/tokens untouched. Measured (arm64, threads=2): single-utt RTF 0.041→0.033, 102.5s audio 0.055→0.049, batch 0.271→0.136; WER on 200 LibriSpeech test-clean 2.42%→2.33% (no regression). Re-export route (fresh NeMo 3.0 export + int4/QKV-merge) was evaluated and gave NO extra gain — int4 MatMulNBits loses to int8 at M=48 GEMM, QKV merge neutral. Work dirs: `~/parakeet-test/` (models, script, 200-wav eval set, `compare_acc.py`), `~/parakeet-export/` (re-export experiments, venv2 with onnx/onnxruntime)
