...
- `docs/fireredasr-cuda-optimization.md` — **CUDA platform companion** (GTX 1660 SUPER server): why split-format int8 fails on CUDA EP (535 host↔device memcpys, GPU only 13% faster than CPU), fp16 conversion via `convert-fp16.py` (encoder 2.67x vs int8, text-identical), MAX_BATCH=0 flip (batching becomes a loss after fp16 — per-step PCIe tax on KV cache), fp32 encoder swap (2026-09-17: fp16 GEMM is ~6x SLOWER than fp32 on TU116's non-tensor-core cublas — the earlier "47% wall-clock gap" was an ORT profiler misreading, nsys shows GPU 97.8% busy; encoder.fp32.onnx 3.1GB deployed, RTF 0.098→0.036, md5 byte-identical; decoder stays fp16 because N=1 decode is bandwidth-bound), BFC arena opt-out `SHERPA_ONNX_CUDA_USE_ARENA=0` (raw cudaMalloc env allocator, `c95b3d8e` — first CUDA code change; fixes fp32 3h48 OOM at 56%: 797s vs fp16 1720s, VRAM sawtooth 4337-4797 MiB instead of ever-growing, e2e cost +2.8%), dead ends (incl. CudaMempoolArena — crashes in production), and the remaining CUDA roadmap (batch ADOPTED 2026-09-18: batch path was already GPU-resident — the "per-step PCIe tax" premise was wrong; real blocker is fp16 decoder GEMM at N≥4; landed via fp32 decoder + length-aware batching + server-side hard-split at 20s (VAD max_speech_duration is a SOFT cap — 27.65s observed — so a hard split was required): 160s RTF 0.027, 1.4x, md5-identical; 3h48 858s (7% slower than fp16-decoder serial but 42% faster on short audio), VRAM peak 5420 MiB, no OOM; remaining: decoder weight-bandwidth reduction). zh chain cumulative: RTF 0.318 → 0.027 (11.8x), 3h48 RTF 0.061, iOS MLX 0.125 far behind
- `docs/qwen3-asr-cuda-optimization.md` — **Qwen3-ASR (other chain) CUDA optimization report** (2026-09-18, commits `02235571..3e3b28ea`): split-format int8 ping-pongs on CUDA EP (encoder 110 MMI+74 DQL, decoder 197 MMI+856 Cast; CPU 4t was 3x faster than int8+CUDA) + upstream kept the 28-layer KV cache on CPU (~470MB/step H2D). Three phases, all text-verified: fp32 re-export via Wasser1462/Qwen3-ASR-onnx + CreateOrtEnv arena-off fix (RTF codeswitch 0.709→0.398 / en_90s 0.815→0.503) → GPU-resident KV via IOBinding, D2D delta write-back (→0.136/0.1915, cumulative 5.2x/4.3x, byte-identical to fp32 baseline; int8→fp32 token divergence approved by user, fp32 == PyTorch reference) → adaptive KV `min(max_total_len, context_len+max_new_tokens+8)` (RTF flat, VRAM peak −170~220 MiB, per-stream KV 470MB→on-demand). 12.5min en job RTF 0.158, peak 5135 MiB. Open risks: direct-decode OOM ceiling at ~65-75s (prefill logits [1,seq,151936]) vs 60s VAD soft cap (thin margin — other chain needs a hard split), CPU-only build link breakage from unconditional extern "C" CUDA symbols (c95b3d8e debt + this change), audio_features ~6MB/step residual H2D. Install gotcha: venv loads `sherpa_onnx/lib/_sherpa_onnx*.so`, NOT the package-root copy
