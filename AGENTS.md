...
- `docs/fireredasr-cuda-optimization.md` — **CUDA platform companion** (GTX 1660 SUPER server): why split-format int8 fails on CUDA EP (535 host↔device memcpys, GPU only 13% faster than CPU), fp16 conversion via `convert-fp16.py` (encoder 2.67x, text-identical), MAX_BATCH=0 flip (batching becomes a loss after fp16 — per-step PCIe tax on KV cache), dead ends, and the remaining CUDA roadmap (attention fusion, GPU-resident KV cache, CUDA graphs, TensorRT). zh chain cumulative: RTF 0.318 → 0.098 (3.2x), beating iOS MLX 0.125
