...
class OfflineFireRedAsrModel {
 public:
   ...
   *                     (num_decoder_layers, N, T, d_model)
   *  - enc_mask: A 2-D tensor of shape (N, T) with 1 for valid frames and
   *              0 for padded frames. It is an empty Ort::Value if the
   *              encoder model does not have this output (e.g., models
   *              exported before the cross_mask support was added).
   */
   ...
   * @param cross_mask A float tensor of shape (N, T) with 1 for valid and
   *                   0 for padded encoder frames. Used only if the decoder
   *                   model has a "cross_mask" input; otherwise it is
   *                   ignored. It can be an empty Ort::Value, in which case
   *                   an all-ones mask is created for such decoders.
   *
   ...
   *  - out_cross_mask Same as cross_mask
   */
